...
import com.example.bookmarket.user.exception.UserEmailAlreadyExistsException;
...
public class UserService {
    ...
    public UserDTO save(UserDTO dto) {
        ...
        userRepository.findByEmail(dto.getEmail()).ifPresent(user -> { // 이메일로 중복된 사용자가 있는지 확인
            throw new UserEmailAlreadyExistsException(USER_EMAIL_ALREADY_EXISTS); // 중복된 사용자가 있으면 예외 발생
        });
    ...
    public UserDTO update(Long id, UserDTO dto) {
        ...
        return UserDTO.fromEntity(userRepository.save(user.toBuilder() // toBuilder()를 사용해서 기존 user를 복사하면서 변경
                .username(dto.getUsername() != null ? dto.getUsername() : user.getUsername()) // 사용자 이름 수정
                .password(dto.getPassword() != null ? passwordEncoder.encode(dto.getPassword()) : user.getPassword()) // 비밀번호 수정
//                .email(...) // 이메일 수정은 보통 없음
                .role(dto.getRole() != null ? dto.getRole() : user.getRole()) // 역할 수정
                .isActive(dto.getIsActive() != null ? dto.getIsActive() : user.getIsActive()) // 활성화 상태 수정
                .build()
        ));
    }
